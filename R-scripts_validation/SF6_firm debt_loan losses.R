...
sdDebt <- aggregate(run[, "loanLiability"], list(run$time), sd)

default <- aggregate(run[, "creditDefault"], list(run$time), mean) #already aggregated

#rename time
mtm <- mtm %>% rename(time = Group.1)

#merge
mtm$default <- default$creditDefault
mtm$sdDebt <- sdDebt$loanLiability
